...
/**
 * Type utilisé pour représenter le résulat d'un pur effet de bord : c'est à dire qui ne produit aucune valeur exploitable
 * par le programme (par exemple écrire quelque chose dans le terminal). Analogue au type {@link Void}, mais évite les
 ...
 *
 * @param <A>
 *            le type de la valeur produite par l'opération d'entrée/sortie lorsqu'elle est exécutée (méthode
 *            {@link IO#run()})
 */
@FunctionalInterface
interface IO<A> {
    ...
    /**
     * Créer une nouvelle opération d'entrée sortie qui utilise de la valeur de retour originale pour une opération IO
     * suivante.
     *
     * @param f
     *            la fonction de transformation de la valeur de retour
    ...
    /**
     * Créer une opération IO à partir d'une valeur simple : garentie de retourné cette même valeur lors de l'exécution
     * (méthode {@link #run()})
     *
     * @param a
